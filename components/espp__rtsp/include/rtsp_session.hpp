...
public:
  ...
  struct Track {
    ...
    espp::UdpSocket rtp_socket;  ///< RTP socket for this track (UDP mode only)
    espp::UdpSocket rtcp_socket; ///< RTCP socket for this track (UDP mode only)
    int client_rtp_port{0};      ///< Client's RTP port (UDP mode)
    int client_rtcp_port{0};     ///< Client's RTCP port (UDP mode)
    bool is_tcp_interleaved{false};  ///< Whether this track uses TCP interleaved transport
    int interleaved_rtp_channel{-1};  ///< RTP interleaved channel number (TCP mode)
    int interleaved_rtcp_channel{-1}; ///< RTCP interleaved channel number (TCP mode)
  ...
  struct Config {
    ...
    std::function<std::string(const std::string &session_path, uint32_t session_id,
        ...
        sdp_generator;
    std::string auth_username;       ///< Digest auth username (empty = auth disabled)
    std::string auth_password;       ///< Digest auth password
    std::string auth_realm;          ///< Digest auth realm (default: "RTSP")
...
protected:
  ...
  /// Looks for transport parameters in the request (either UDP client_port or TCP interleaved)
  /// and returns them. For UDP transport, client_rtp_port and client_rtcp_port are set.
  /// For TCP interleaved transport, is_tcp_interleaved is set to true and interleaved channels are parsed.
  /// @param request The request to parse
  /// @param rtsp_path The RTSP path from the request (output)
  /// @param client_rtp_port The client RTP port number (output, UDP mode only)
  /// @param client_rtcp_port The client RTCP port number (output, UDP mode only)
  /// @param is_tcp_interleaved Whether the transport is TCP interleaved (output)
  /// @param interleaved_rtp_channel RTP interleaved channel (output, TCP mode only)
  /// @param interleaved_rtcp_channel RTCP interleaved channel (output, TCP mode only)
  /// @return True if the request was parsed successfully, false otherwise
  ...
  bool check_auth(std::string_view request, int cseq, const std::string &method);
