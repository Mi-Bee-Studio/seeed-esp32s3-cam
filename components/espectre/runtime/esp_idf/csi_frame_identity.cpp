...
bool csi_frame_matches_traffic(const wifi_csi_info_t *info,
                               const CsiFrameFilterConfig &config) {
  // MiBee patch (2026-09-06, IDF v6.0 pilot): the provenance filter relies on
  // wifi_csi_info_t.payload/payload_len, which the IDF v6.0.1 esp_wifi driver
  // does not fill on ESP32-S3 (upstream works on 5.5.x) — parse_bounded_payload
  // then fails and EVERY callback is rejected (measured: rej == cb, 100%).
  // Bypass the filter: in sensing mode every capture-valid CSI frame is
  // signal. Revisit when the driver fills payload again or upstream adapts.
  (void)info;
  (void)config;
  return true;
}
