...
13 | Web Server | Port 80, registers 16 URI handlers (14 API + 2 wildcard)
13a | OTA Updater | Initializes firmware OTA update capability | `ota_updater_init()`
...
The system has 20 modules, all located in the `main/` directory, each module with one `.c`/`.h` file pair.
...
| Web Server | `web_server.c` | HTTP server + REST API (16 endpoints) | `web_server_start()`, `web_server_get_handle()` |
...
- **Core 1**: Runs upload, SD monitoring, health monitoring, WebSocket server and other non-real-time tasks
...
> **Note**: Uses factory partition for OTA updates via `esp_https_ota`, no separate OTA partition needed.

---

## 7. Web API Endpoints

Server runs on port 80, total 16 URI handlers:
