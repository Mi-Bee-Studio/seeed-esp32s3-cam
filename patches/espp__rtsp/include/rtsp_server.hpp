...
public:
  ...
  struct Config {
    ...
    size_t control_task_stack_size_bytes =
        RtspSession::Config::default_control_task_stack_size_bytes; ///< Per-session RTSP
                                                                    ...
                                                                    ///< bytes
    std::string auth_username;       ///< Digest auth username (empty = disabled)
    std::string auth_password;       ///< Digest auth password
    std::string auth_realm;          ///< Digest auth realm
    /// Maximum number of concurrent sessions. 0 = unlimited (library default).
    /// New connections beyond this limit are rejected at accept time.
    /// MiBee Cam patch: caps memory/socket usage on resource-constrained ESP32-S3.
  ...
  /// @brief Get the number of connected sessions.
  /// @return Number of sessions tracked by the server.
  size_t get_num_sessions() const;
